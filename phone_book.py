...
db_connect = pymysql.connect(
    ...
    db ='test_phone_book', 
    charset = 'utf8',
    cursorclass = pymysql.cursors.DictCursor)  # SELECT의 결과를 tuple이 아닌 list형태로 가져오도록


# 쿼리를 날려주는 역할
cursor = db_connect.cursor()

# 로그인한 사용자가 몇 번 사용자인지 변수로 저장
...
def sign_in():
    ...
    if len(user_list)  == 0:
        ...
        print(login_user)   # 로그인 사용자 정보 => 모든 항목들을 dict로 묶어서 들고있다.
        user_nickname = login_user['nickname']  # 사용자 정보 tuple에서 닉네임을 추출
        
        global login_user_id  # 최상단에서 만든 login_user_id  변수를 끌어와서 사용하겠다고 명시
        login_user_id = login_user['id']  # 로그인한 사용자(내가) 몇 번 id를 가지고 있는 지 추출
...
def add_phone_num():
    ...
    # 1. DB에 넣어야 할 항목들을 입력받자
    input_name = input('이름 : ')
    input_phone_num = input('전화번호 : ')
    input_memo = input('메모사항 : ')
    
    print('로그인한 사용자 아이디 : ', login_user_id)
    # 2. SQL 작성 -> INSERT INTO로 데이터 추가 => user_id=12 처럼 DB의 본인 숫자를 찾아서 하드코딩으로 INSERT
    ...
    # 3. cursor / db_connect를 이용, 실제 DB에 쿼리 수행
    cursor.execute(sql)
    db_connect.commit()
...
def search_my_contact_list():
    ...
    result = cursor.fetchall()  # DB row들의 목록 : 무조건 tuple 형태로 나옴
    ...
    if len(result) == 0:
        ...
        # 위치에 맞는 dict 꺼내오자 => dict의 내용을 가지고 => contact 객체의 내용물 변수들을 채워주자 (클래스의 기능 추가)
        select_line = result[contact_num - 1]  # 위치에 맞는 dict 꺼내오기
        contact.set_data(select_line)  # dict -> 클래스 인스턴스 예제
        
        # cf) 클래스 객체 -> dict로 변환해야 하는 경우도 많다. (메쏘드로 만들고 활용. 향후 체험 예정)
